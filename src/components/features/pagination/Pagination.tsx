...
export const Pagination = ({ currPage, count, limit }: IProps) => {
  ...
  // const pages = Array.from({ length: offset }, (_, index) => index + 1);
  ...
  return (
    <div className="flex w-full items-center justify-between gap-1 p-4 text-text-highlight md:justify-center md:gap-4">
      <button
        disabled={currPage === 1}
        onClick={() => changePage(currPage - 1)}
        className="cursor-pointer rounded-2xl bg-secondary p-2 disabled:cursor-not-allowed disabled:opacity-50"
      >
        <ChevronLeft size={20} className="stroke-text-highlight" />
      </button>
      <div className="flex flex-wrap items-center gap-1 gap-y-1 text-sm md:gap-2">
        {pages.map((page, i) =>
          page === "..." ? (
            ...
            <button
              key={page}
              className={`aspect-square h-9 w-9 overflow-hidden rounded-2xl p-2 ${
                currPage === page
                  ? "bg-secondary-accent"
      ...
      <button
        ...
        className="cursor-pointer rounded-2xl bg-secondary p-2 disabled:cursor-not-allowed disabled:opacity-50"
      >
        <ChevronRight size={20} className="stroke-text-highlight" />
      </button>
    </div>
  );
};
