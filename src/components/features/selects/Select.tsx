...
import type { IOption, ISelectProps } from "@/utils/models/inputs";
...
export const Select: React.FC<ISelectProps> = ({
  ...
  return (
    <div
      ...
      className={`relative w-full min-w-60 text-text-primary ${isDisabled ? "pointer-events-none opacity-50" : ""}`}
    >
      <div
        className="flex min-h-11.5 cursor-pointer items-center justify-between gap-0.5 rounded-lg border border-border bg-white px-3 py-2.5 hover:border-secondary-accent"
      ...
      >
          ...
          {!isEmpty ? (
            isMulti ? (
              selectedArray.map((v) => (
                <span
                  key={v.value}
                  className="flex items-center gap-1 rounded bg-gray-100 py-0.5 pr-1 pl-2 text-sm"
                  style={v.color ? { backgroundColor: v.color } : {}}
                  onClick={(e) => e.stopPropagation()}
                >
                  {v.label}
                  <X
                    className="transition-colors hover:stroke-secondary-accent"
                    size={12}
        ...
        <div className="flex items-center gap-1">
          {isLoading && (
            <Loader2 size={16} className="animate-spin stroke-text-primary" />
          )}
          {isClearable && selectedArray.length ? (
            <>
              <X
                size={16}
                className="text-text-primary transition-colors hover:stroke-secondary-accent"
                onClick={handleClear}
              />
              <span className="my-0.5 flex w-[1px] self-stretch bg-text-secondary" />
            </>
          ) : null}
          <ChevronDown
            size={18}
            className={`transform transition-transform hover:stroke-secondary-accent ${isOpen ? "rotate-180" : "rotate-0"}`}
          />
        </div>
      </div>
      ...
      {isOpen &&
        createPortal(
          <div
            ...
            className={`mt-1 w-full overflow-hidden rounded-lg border border-gray-300 bg-white shadow-lg ${
          ...
          >
            <div className="max-h-60 overflow-auto">
              {filteredOptions.map((option) => (
                <div
                  key={option.value}
                  className={`flex cursor-pointer items-center justify-between px-3 py-2 select-none hover:bg-gray-100 ${
                ...
                >
                  {option.label}
                  {isSelected(option) && (
                    <CheckCheck size={16} className="text-secondary-accent" />
                  )}
                </div>
              ))}
            </div>
            {filteredOptions.length === 0 && (
              <div className="px-3 py-2 text-center">No results</div>
            )}
          </div>,
